...
WatsonDiscoverySetup.prototype.findDiscoveryConfig = function(params) {
  return new Promise((resolve, reject) => {
    this.discoveryClient.listConfigurations(params, (err, data) => {
      if (err) {
        console.error(err);
        return reject(new Error('Failed to get list of Discovery configurations.'));
      } else {
        ...
        for (let i = 0, size = configs.length; i < size; i++) {
          ...
          if (config.name === 'Default Configuration') {
            ...
            console.log('Use default configuration: ' + config.configuration_id);
          } else if (config.name === params.config_name) {
            // found the discovery config, so check if it is setup correctly
            console.log('Found discovery config with name: ' + config.name);
            console.log('Need to verify it is setup correctly');

            console.log(JSON.stringify(config, null, 2));
            if (config.enrichments && config.enrichments[0].options.features.entities.model) {
              // it is good, so use it
              params.configuration_id = config.configuration_id;
              return resolve(params);
            } else {
              // if is bad, so we need to update it
              params.default_config_id = config.configuration_id;
            }
          }
        }

        // no valid config found, so create new one, or update the existing one
        console.log('No valid discovery config found, so create a new one');
        return resolve(params);
      }
    });
  });
...
WatsonDiscoverySetup.prototype.getDiscoveryConfig = function(params) {
  return new Promise((resolve, reject) => {
    this.discoveryClient.getConfiguration(params, (err, configuration) => {
      if (err) {
        console.error(err);
        ...
        if (configuration.name === 'Default Configuration') {
          ...
          // that means we already have set this or there was a previous error we need to clean up.
          ...
          if (configuration.enrichments && configuration.enrichments[0].options.features.entities.model) {
            ...
            // if is bad, so we need to update
...
WatsonDiscoverySetup.prototype.updateDiscoveryConfig = function(params) {
  return new Promise((resolve, reject) => {
    ...
    params.configuration_id = params.default_configuration.configuration_id;
        ...
        for (let i = 0, size = collections.length; i < size; i++) {
          ...
          if (validateID) {
            if (validateID === collection.collection_id) {
              ...
              params.configuration_id = collection.configuration_id;
              return resolve(params);
            }
