...
const PRODUCT_DATA_INDEX     = 1;
const REVIEWER_DATA_INDEX    = 2;
const CATEGORY_DATA_INDEX    = 3;
const CONCEPT_DATA_INDEX     = 4;
const KEYWORD_DATA_INDEX     = 5;
const ENTITY_TYPE_DATA_INDEX = 6;
...
function formatData(data, filterString) {
  ...
  let results = data.rawResponse.result.results;
  results.forEach(function(dataItem) {
    ...
    if (dataItem.hasOwnProperty('enriched_text')) {
      ...
      if (dataItem.enriched_text[0].sentiment) {
        if (dataItem.enriched_text[0].sentiment['score']) {
          sentimentScore = dataItem.enriched_text[0].sentiment.score;
        }
        if (dataItem.enriched_text[0].sentiment['label']) {
...
function getTotals(data) {
  ...
  // this "data" is created by formatData()
